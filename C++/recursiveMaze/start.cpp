...
// This counts the number of paths to the origin on an empty grid
unsigned int gridCounter(int x, int y) {
  // Found origin
  if (x == 0 && y == 0){
    return 1;
  }
  ...
  // At the edge
  if (x == 0) {
    return gridCounter(x, y-1);
  }
  else if (y == 0) {
    return gridCounter(x-1, y); 
  }
  
  // Recurse both directions (don't go down or right as going away from origin)
  return gridCounter(x, y-1) + gridCounter(x-1, y);
 
}

// Main function to make recursive.
int mainF() {
  // Decide path down function
  string choice;
  cout << endl << "Enter what mode you want to run (counter, grid, exit): ";
  cin >> choice;

  if (choice == "grid") {
    string input;
    cout << "Enter the file name for the grid: ";
    cin >> input;
    ...
    cout << endl << "Enter starting x: ";
    int x;
    cin >> x; 
    cout << "Enter starting y: ";
    int y;
    cin >> y;
    unsigned int result = gridCounter(x, y);
    cout << "Number of paths on grid: " << result << endl;
    return mainF();
  } 
    ...
    cout << "Invalid choice picked. Please try again." << endl;
    return mainF();
  }
